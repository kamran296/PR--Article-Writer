...
module.exports.loaOrgAddData = async (req, res) => {
  try {
    const { question, answer } = req.body;
    console.log(question, answer);
    const original = new loaOrg({
      messages: [
        { role: "system" },
...
const download = async () => {
  const originalData = await loaOriginal.find().lean();

  // Remove the _id field from each document

  const originalDataWithoutId = originalData.map((doc) => {
  ...
  if (!originalDataWithoutId || originalDataWithoutId.length === 0) {
    throw new Error("No chat data found");
  }

  // Convert the data to a JSON string
  const jsonData = JSON.stringify(originalDataWithoutId, null, 2);
  ...
  const filePath = path.join(dataDir, "originalData.json");
...
module.exports.fineTune = async (req, res) => {
  ...
  try {
    ...
    const Path = path.join(__dirname, "../data/originalData.json");
